...
for i in range(len(rows[0])):
    ...
    colsums = [sum(col) for col in cols]
print("Col sums:", colsums)

maxlen = len(str(max(max(col) for col in cols)))

print("\nRow sorted:")
for total, row in sorted(zip(rowsums, rows)):
    print(" ".join(str(x).zfill(maxlen) for x in row))

print("\nCol sorted:")
colsortCols = [col for total, col in sorted(zip(colsums, cols))]
colsortRows = [[] for _ in range(len(rows))]
for col in colsortCols:
    for i in range(len(rows)):
        colsortRows[i].append(col[i])
for row in colsortRows:
    print(" ".join(str(x).zfill(maxlen) for x in row))
